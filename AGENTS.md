...
- **Single-player prompt injection game**: 8 levels, local regex matching only — no LLM/backend
...
| `app/(tabs)/_layout.tsx` | Tab bar config (Home, Ranks, Learn, Quests) |
...
- Dev server uses `bunx rork start` with hardcoded project ID, not plain `expo start`
- `bunx` may not be in PATH; use `bun run start` instead
- `server.ts` is an untracked dev artifact — ignore
- GitHub Pages deployment lives on `gh-pages` branch (web export in `expo/dist/`)
